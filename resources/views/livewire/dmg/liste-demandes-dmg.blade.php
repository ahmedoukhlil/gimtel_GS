    ...
    <div>
        ...
        <div class="flex flex-wrap items-end justify-between gap-4 mt-4">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Gerez et traitez les demandes des services internes</p>
            </div>
    ...
    <div class="flex flex-wrap items-center gap-3">
        <div class="relative max-w-md flex-1 min-w-[200px]">
            ...
            <input type="text" wire:model.live.debounce.300ms="search"
                   ...
                   placeholder="Rechercher par n°, service ou demandeur...">
            @if($search !== '')
        ...
        <select wire:model.live="filterDemandeurId"
                class="rounded-lg border-gray-200 text-sm py-2.5 px-3 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white">
            <option value="">Tous les demandeurs</option>
            @foreach($demandeurs as $dem)
                <option value="{{ $dem->id }}">{{ $dem->nom }} ({{ $dem->poste_service }})</option>
            @endforeach
        </select>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead>
                    <tr class="bg-gray-50/80">
                        <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">N°</th>
                        <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Demandeur</th>
                        <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Saisi par</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($demandes as $d)
                        ...
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-5 py-4"><span class="text-sm text-gray-500">{{ $d->demandeur->users ?? '—' }}</span></td>
                        ...
                        <tr>
                            <td colspan="7" class="px-5 py-16 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    @if($search !== '' || $filterStatut !== '' || $filterDemandeurId)
                                        ...
                                        <button wire:click="$set('search', ''); $set('filterStatut', ''); $set('filterDemandeurId', null)" class="mt-3 text-sm text-indigo-600 hover:text-indigo-800 font-medium">Reinitialiser</button>
                                    @else
                                        <p class="text-sm font-medium text-gray-500">Aucune demande d'approvisionnement</p>
                                        <p class="text-xs text-gray-400 mt-1">Les demandes des services apparaitront ici</p>
                                    @endif
                                </div>
                            </td>
                        </tr>
